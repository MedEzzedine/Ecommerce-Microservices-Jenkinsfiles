                    ...
                    steps {
                        script {
                            for (microservice in microservices) {
                                ...
                                sh "docker run -it --rm aquasec/trivy -q image -f html $DOCKERHUB_USER/$microservice:$BRANCH_NAME-$BUILD_NUMBER > trivy-report-${microservice}.html"
                                archiveArtifacts "trivy-report-${microservice}.html"
                            }
                            
                            sh "docker run -it --rm aquasec/trivy -q image -f html $DOCKERHUB_USER/ecomm-frontend:$BRANCH_NAME-$BUILD_NUMBER > trivy-report-ecomm-frontend.html"
                            archiveArtifacts "trivy-report-${microservice}.html"
                        }
                    }
                }
