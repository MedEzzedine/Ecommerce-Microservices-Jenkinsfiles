                    ...
                    steps {
                        script {
                            for (def microservice in microservices) {
                                ...
                                sh(script: 'docker run --privileged --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image $DOCKERHUB_USER/${microservice}:$BRANCH_NAME-$BUILD_NUMBER | awk \'{print "<tr><td>" $1 "</td><td>" $2 "</td><td>" $3 "</td></tr>"}\' > trivy-report-${microservice}.html', returnStdout: true)
                                archiveArtifacts "trivy-report-${microservice}.html", allowEmptyArchive: true
                            }
                            
                            sh(script: 'docker run --privileged --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image $DOCKERHUB_USER/ecomm-frontend:$BRANCH_NAME-$BUILD_NUMBER | awk \'{print "<tr><td>" $1 "</td><td>" $2 "</td><td>" $3 "</td></tr>"}\' > trivy-report-ecomm-frontend.html', returnStdout: true)
