                    ...
                    steps {
                        script {
                            for (def microservice in microservices) {
                                ...
                                sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $PWD:/tmp/.cache/ aquasec/trivy image --format template --template \$(curl -o html.tpl https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/html.tpl && echo html.tpl) $DOCKERHUB_USER/${microservice}:$BRANCH_NAME-$BUILD_NUMBER > trivy-report-${microservice}.html"
