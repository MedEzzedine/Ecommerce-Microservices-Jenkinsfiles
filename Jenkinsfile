                    ...
                    steps {
                        script {
                            ...
                            for (def microservice in microservices) {
                                ...
                                sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $PWD:/tmp/.cache/ -v ./html.tpl:/tmp/html.tpl aquasec/trivy image --format template --template /tmp/html.tpl $DOCKERHUB_USER/${microservice}:$BRANCH_NAME-$BUILD_NUMBER > trivy-report-${microservice}.html"
                                archiveArtifacts artifacts: "trivy-report-${microservice}.html", allowEmptyArchive: true
                            }
                            
                            sh "docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $PWD:/tmp/.cache/ aquasec/trivy image $DOCKERHUB_USER/ecomm-frontend:$BRANCH_NAME-$BUILD_NUMBER | awk '{print \"<tr><td>\" \$1 \"</td><td>\" \$2 \"</td><td>\" \$3 \"</td></tr>\"}' > trivy-report-ecomm-frontend.html"
